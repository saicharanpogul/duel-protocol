...
pub fn price(supply: u64, params: &CurveParams) -> Result<u64, ProgramError> {
    let k = supply as u128;
    let a = params.a as u128;
    let b = params.b as u128;

    let k_pow_n = pow_u128(k, params.n).ok_or(ProgramError::ArithmeticOverflow)?;
...
pub fn reserve_integral(supply: u64, params: &CurveParams) -> Result<u128, ProgramError> {
    let k = supply as u128;
    let a = params.a as u128;
    let b = params.b as u128;
    let n_plus_1 = (params.n as u128)
        .checked_add(1)
        .ok_or(ProgramError::ArithmeticOverflow)?;

    let k_pow_n1 = pow_u128(k, params.n + 1).ok_or(ProgramError::ArithmeticOverflow)?;
