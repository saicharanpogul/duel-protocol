...
impl<'info> SellPostResolution<'info> {
    #[inline(always)]
    pub fn handler(
        &mut self,
        side: u8,
        ...
        // Must be emergency resolution (no winner)
        require!(self.market.has_winner == 0, DuelError::EmergencyOnlyOperation);
        // Re-entrancy check
        ...
        // Calculate quote out (no trade fees on emergency sells)
        let params = CurveParams::default_params();
        let quote_amount = math::sol_out(token_amount, current_supply, &params)?;
